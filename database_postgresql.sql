...
-- PostgreSQL Version for managing IPALD (Instalasi Pengolahan Air Limbah Domestik) facilities

-- Create database (PostgreSQL uses CREATE DATABASE separately)
-- CREATE DATABASE si_pal;
-- \c si_pal;

-- Drop existing tables if they exist (for clean recreation)
DROP TABLE IF EXISTS activity_logs CASCADE;
DROP TABLE IF EXISTS users CASCADE;
DROP TABLE IF EXISTS pengelolaan_air_limbah CASCADE;

-- Create ENUM types first (PostgreSQL requires custom types)
CREATE TYPE kondisi_status_type AS ENUM (
    'Bangunan Baik, Optimal', 
    'Bangunan Baik, Tidak Optimal', 
    'Bangunan Rusak, Optimal', 
    'Bangunan Rusak, Tidak Optimal'
);

CREATE TYPE pengecekan_effluent_type AS ENUM (
    'Belum dilakukan', 
    'Sudah dilakukan', 
    'Dalam proses'
);
...
CREATE TABLE pengelolaan_air_limbah (
    ...
    kapasitas_desain INTEGER NOT NULL,
    kapasitas_terpasang INTEGER NOT NULL,
    tahun_dibangun_rehabilitasi INTEGER, -- Changed from YEAR to INTEGER
    kondisi_status_operasional kondisi_status_type NOT NULL,
    lembaga_pengelola VARCHAR(100) NOT NULL,
    pengecekan_effluent pengecekan_effluent_type NOT NULL,
...
-- Create indexes
CREATE INDEX idx_kelurahan ON pengelolaan_air_limbah(kelurahan_desa);
CREATE INDEX idx_lembaga ON pengelolaan_air_limbah(lembaga_pengelola);
CREATE INDEX idx_kondisi ON pengelolaan_air_limbah(kondisi_status_operasional);
CREATE INDEX idx_koordinat ON pengelolaan_air_limbah(latitude, longitude);

-- Create trigger for updated_at (PostgreSQL doesn't have ON UPDATE CURRENT_TIMESTAMP)
...
CREATE TRIGGER update_pengelolaan_air_limbah_updated_at 
    BEFORE UPDATE ON pengelolaan_air_limbah 
...
CREATE TABLE users (
    ...
    role user_role_type DEFAULT 'operator',
...
CREATE TABLE activity_logs (
    ...
    record_id INTEGER,
    old_values JSONB, -- Changed from JSON to JSONB for better performance
    new_values JSONB, -- Changed from JSON to JSONB for better performance
...
CREATE INDEX idx_table_record ON activity_logs(table_name, record_id);
CREATE INDEX idx_created_at ON activity_logs(created_at);

-- Create trigger for updated_at on activity_logs table
...
SELECT 
    ...
    pengecekan_effluent,
    CASE 
        WHEN kapasitas_terpasang > 0 THEN ROUND((kapasitas_terpasang::decimal / kapasitas_desain::decimal) * 100, 2)
        ELSE 0
...
-- Create view for facility statistics
CREATE VIEW facility_statistics AS
SELECT 
...
-- GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO sipal_user;
-- GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO sipal_user;

-- Comments for documentation
COMMENT ON TABLE pengelolaan_air_limbah IS 'Table for storing wastewater treatment facility data (IPALD)';
COMMENT ON COLUMN pengelolaan_air_limbah.nama IS 'Nama fasilitas IPALD';
COMMENT ON COLUMN pengelolaan_air_limbah.kelurahan_desa IS 'Kelurahan/Desa yang masuk dalam cakupan pelayanan';
COMMENT ON COLUMN pengelolaan_air_limbah.kapasitas_desain IS 'Kapasitas Desain (SR)';
COMMENT ON COLUMN pengelolaan_air_limbah.kapasitas_terpasang IS 'Kapasitas Terpasang (SR)';
COMMENT ON COLUMN pengelolaan_air_limbah.tahun_dibangun_rehabilitasi IS 'Tahun Dibangun/Rehabilitasi';
COMMENT ON COLUMN pengelolaan_air_limbah.kondisi_status_operasional IS 'Kondisi dan Status Operasional';
COMMENT ON COLUMN pengelolaan_air_limbah.lembaga_pengelola IS 'Lembaga Pengelola (BLUD, Swasta, dll)';
COMMENT ON COLUMN pengelolaan_air_limbah.pengecekan_effluent IS 'Status Pengecekan Effluent';
COMMENT ON COLUMN pengelolaan_air_limbah.latitude IS 'Koordinat Latitude';
COMMENT ON COLUMN pengelolaan_air_limbah.longitude IS 'Koordinat Longitude';
COMMENT ON COLUMN pengelolaan_air_limbah.alamat_lengkap IS 'Alamat lengkap fasilitas';
COMMENT ON COLUMN pengelolaan_air_limbah.keterangan IS 'Keterangan tambahan';

COMMENT ON TABLE users IS 'Table for storing user authentication and authorization data';
COMMENT ON TABLE activity_logs IS 'Table for storing user activity logs for audit purposes';
